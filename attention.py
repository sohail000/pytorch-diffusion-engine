...
def get_attention_implementation():
    ...
    Priority: Flash Attention (PyTorch 2.0+) > Standard
    """
    if hasattr(F, "scaled_dot_product_attention"):
        return "flash"  # PyTorch 2.0+ native
    return "standard"
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, causal_mask=False):
        ...
        # (Batch_Size, Seq_Len, Dim) -> 3x (Batch_Size, Seq_Len, Dim)
        ...
        if ATTN_IMPL == "flash" and hasattr(F, "scaled_dot_product_attention"):
            # PyTorch 2.0+ Flash Attention — O(N) memory, much faster
